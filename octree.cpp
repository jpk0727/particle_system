...
//Some comments added by Zoe Junghans and Jess Karol, also CheckMutualCollisions method has been modified from the original.
...
{
    //--
    int Octree::s_i32MaxDepth = 20;
    float Octree::s_f32MaxObjectNodeRatio = 5.0f;
    ...
    //Updates the positions of all the ISpatialObjects
    void
    Octree::VUpdate()
    {   
    ...
    //Adds the passed in vector of objects into the octree
    void
    ...
    //Some particle-particle collisions handled here
    //Checks collisions between the object passed in and the other objects in its node
    void
    OctreeNode::CheckMutualCollisions(ISpatialObject* pObject)
    {
        ...
        while(pIter1)
        {
            ...
            while(pIter2)
            {
                ...
                //Checks if there is a collisions
                if(pIter1->VCheckCollision(pIter2, true))
                {
            ...
            /*pIter2 = m_pObjects;
            while(pIter2){
                if(pIter1->VCheckCollision(pIter2, false)){
                    pIter1->VApplyPressure(counter, pIter2);
                }
                pIter2 = pIter2->VGetNext();
            }*/

            //Turns the collision status off it the particle has not been hit in this iteration
            if(!hit){
                pIter1->VCollisionOff();
            }
